...
export default function DetallePelicula() {
  ...
  if (loading) return <main className="page-shell"><p className="empty-state">Cargando película...</p></main>;
  ...
  return (
    <main className="page-shell">
      <section className="grid gap-8 lg:grid-cols-[320px_1fr]">
        {poster ? (
          <img className="rounded-lg object-cover shadow-soft" src={poster} alt={`Poster de ${pelicula.titulo}`} />
        ) : (
        ...
        <div className="space-y-6">
          ...
          <p className="max-w-3xl text-lg leading-8 text-slate-700">{pelicula.sinopsis}</p>

          <section className="space-y-3">
            <h2 className="text-2xl font-black text-night">Funciones disponibles</h2>
            {!pelicula.funciones?.length && <p className="empty-state">Todavía no hay funciones para esta película.</p>}
            <div className="grid gap-3">
              {pelicula.funciones?.map((funcion) => (
                <article key={funcion.id} className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="font-black text-night">{formatDate(funcion.fechaHora)}</p>
                    <p className="text-sm font-semibold text-slate-600">
                      {funcion.sala?.nombre} · ${formatMoney(funcion.precio)}
                    </p>
                  </div>
                  <Link className="btn-primary text-center" to={`/reserva/${funcion.id}`}>Reservar</Link>
                </article>
              ))}
            </div>
          </section>
